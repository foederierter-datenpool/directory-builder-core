...
test("the webapp indexes public data artifacts and builds a directory tree", () => {
    ...
    assert.ok(files.includes("data/pipeline/final.ttl"))
    assert.ok(files.every((file) => !file.split("/").some((part) => part.startsWith("."))))

    const tree = buildDataFileTree([
        "data/pipeline/final.ttl",
        "data/pipeline/provenance.ttl",
        "data/ingest/raw/source/data file.json",
    ])
    assert.deepEqual([...tree.directories.keys()], ["ingest", "pipeline"])
    assert.deepEqual(tree.directories.get("pipeline").files.map((file) => file.name),
        ["final.ttl", "provenance.ttl"])
